...
public class Client {
	...
	private void importTreeContent() {
		...
		contactTree.getParent().redraw();
		contactTree.getParent().layout();
	}
...
//			dateOfBirthDateTime.setData(selectedContact.getDateOfBirth());
//			dateOfBirthDateTime.setEnabled(true);
	...
	private void refreshTreeAfterDeletingContact() {
		Contact deletedContact = (Contact) contactTree.getSelection()[0].getData();
...
//		firstNameLbl.setText(Contact.Field.FIRST_NAME.getFieldName());
		...
		phoneNumText.setEnabled(false);
		phoneNumText.addModifyListener(listener);
		...
		dateOfBirthText.setEnabled(false);
		dateOfBirthText.addModifyListener(listener);
		...
		zipCodeText.setEnabled(false);
		zipCodeText.addModifyListener(listener);
	...
	private void updateContact() {
		...
		if (!contactToUpdate.getPhoneNumber().contains(newPhoneNumber)) {
			List<String> newPhoneNumList = contactToUpdate.getPhoneNumber();
...
//			List<Address> newAddressList = contactToUpdate.getAddress();
